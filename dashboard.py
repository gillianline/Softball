...
if check_password():
    ...
    with tab_profile:
        ...
        if isinstance(p_dates, tuple) and len(p_dates) == 2:
            ...
            # --- 2. CALCULATE AGGREGATE SCORE & STATUSES ---
            # Defaults
            rsi = p_c['RSI-modified (Imp-Mom) [m/s]'].mean() if not p_c.empty else 0.35
            maint_pct = (p_ash['Peak Vertical Force [N]'].mean() / ash_filt['Peak Vertical Force [N]'].max() * 100) if not p_ash.empty else 90
            asym = (abs(p_ash['Peak Vertical Force [N] (L)'].mean() - p_ash['Peak Vertical Force [N] (R)'].mean()) / p_ash['Peak Vertical Force [N] (L)'].mean() * 100) if not p_ash.empty else 5
            
            # The "Performance Score" Formula
            perf_score = (maint_pct * 0.4) + ((rsi / 0.5) * 40) + ((100 - asym) * 0.2)
            
            # Status Logic
            readiness = "PEAKING" if rsi > 0.45 else "STABLE" if rsi > 0.35 else "FATIGUED"
            load_val = p_s.shape[0] + p_t.shape[0]
            load_status = "HIGH" if load_val > 10 else "MODERATE" if load_val > 4 else "LOW"
            asym_status = "LOW" if asym < 10 else "MODERATE" if asym < 15 else "HIGH"

            st.divider()

            # --- 3. ATHLETE STATUS (The "Bottom Line") ---
            st.markdown("### ATHLETE STATUS")
            s1, s2, s3, s4, s5 = st.columns(5)
            s1.metric("Readiness", readiness)
            s2.metric("Load", load_status)
            s3.metric("Asymmetry", asym_status)
            s4.metric("Score", f"{perf_score:.0f}")
            
            # Trend Arrow Logic (Comparing to previous 7 days)
            prev_rsi = cmj_filt[cmj_filt['Date'].dt.date < start_p]['RSI-modified (Imp-Mom) [m/s]'].tail(3).mean()
            rsi_trend = "↑" if rsi > prev_rsi else "↓"
            s5.metric("CNS Trend", f"{rsi:.2f} {rsi_trend}")
        ...
        with tab_swing:
            if not swing_df.empty:
                ...
                with f1:
                    ...
                    selected_dates_s = st.date_input("Select Date Range", value=(min_s.date(), max_s.date()), key="swing_date_range_final")
                with f2:
                    s_cat = st.segmented_control("Session Type", options=["All", "Games", "Practices"], default="All", key="s_ct_final")
                ...
                if isinstance(selected_dates_s, tuple) and len(selected_dates_s) == 2:
                    ...
                    if not p_s.empty:
                        ...
                        # Logic for the "Traffic Light" Status
                        if intent_pct > 25:
                        ...
                        # 4. RANGE SUMMARY HEADER & METRICS
                        ...
                        for _, row in hist_s.iterrows():
                            rows_list.append(f"<tr><td>{row['Date']}</td><td>{row['Session Type']}</td><td>{int(row['Total'])}</td><td>{int(row['Max Intent'])}</td><td>{row['Intensity']:.2f}</td><td>{row['Rot_Pct']:.1f}%</td></tr>")
                
                        table_html = f"""
                        <style>
                            .coach-table {{ width: 100%; border-collapse: collapse; font-family: sans-serif; text-align: center; }}
                            .coach-table th {{ background-color: #f8f9fa; padding: 12px; border-bottom: 2px solid #dee2e6; }}
            ...
            # --- GLOSSARY (Always at bottom) ---
            with st.expander("What do these hitting metrics mean?"):
                st.markdown("""
                ### Swing Metric Definitions
                * **Total Swings**: Absolute count of recorded swings.
                * **Max Intent**: Swings reaching **Rotation Band 3** (maximal torso rotation speed).
                * **Load/Sw (Intensity)**: Player Load divided by Swings. Measures explosiveness per rep.
                * **Rot % (Rotation Style)**: Energy from rotation vs. forward movement. Higher = more rotational.
