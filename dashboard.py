        ...
        with tab_testing:
            ...
            if not p_cmj_ready.empty and cmj_h_col and cmj_rsi_col and cmj_h_col in p_cmj_ready.columns and cmj_rsi_col in p_cmj_ready.columns:
                ...
                if not p_cmj_ready.empty:
                    # Group by Date to average multiple reps on the same day
                    ...
                    with c_right:
                        ...
                        # 1. JUMP HEIGHT TRACE (Left Axis: yaxis)
                        fig_cmj.add_trace(
                            go.Scatter(
                                x=cmj_plot_df['Date'],
                        ...
                        # 2. RSI MODIFIED TRACE (Right Axis: yaxis2)
                        fig_cmj.add_trace(
                            go.Scatter(
                                x=cmj_plot_df['Date'],
                        ...
                        # Explicit dual axis ranges to prevent overlay collapsing
                        h_val = float(latest_h)
